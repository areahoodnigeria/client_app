...
import Sidebar, { neighbourLinks, neighbourBottomLinks } from "../../components/dashboard/Sidebar";
...
export default function NeighbourLayout() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-background via-primary/5 to-background relative">
      <Sidebar collapsed={collapsed} links={neighbourLinks} bottomLinks={neighbourBottomLinks} topOffset={0} />

      {/* Main content area padding accounts for fixed sidebar */}
      <main className={`pt-6 transition-all ${collapsed ? "pl-16" : "pl-64"} px-4 md:px-8`}>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mt-6">
            ...
            <div className="lg:col-span-2 space-y-4">
              ...
              <div className="glass-card p-6 md:p-8">
                <h1 className="text-2xl md:text-3xl font-bold text-foreground">
                  Hey {firstName}, <span className="text-gradient">welcome back to your hood!</span>
                </h1>
                <p className="text-muted-foreground mt-2">Here's what's happening in your neighborhood</p>
              </div>
                  ...
                  {posts.map((post, index) => (
                    ...
                    >
                      <div className="flex items-center gap-3 mb-3">
                        <div className={`h-10 w-10 rounded-full bg-gradient-to-br ${post.avatarColor} shadow-inner`} />
                        <div>
                          <div className="font-semibold text-foreground leading-tight">{post.name}</div>
                          <div className="text-xs text-muted-foreground">{post.time} ago</div>
                        </div>
                      </div>
                      <p className="text-sm md:text-base text-foreground leading-relaxed">{post.text}</p>
            ...
            <div className="space-y-4">
              ...
              <div className="bg-white/70 dark:bg-black/30 backdrop-blur-md border border-border rounded-xl shadow-card p-5">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <div className="text-lg font-semibold text-foreground">Doon South</div>
                  </div>
              ...
              <div className="bg-white/70 dark:bg-black/30 backdrop-blur-md border border-border rounded-xl shadow-card p-5">
                ...
                <div className="text-sm text-muted-foreground">No new alerts. Stay safe and informed.</div>
              </div>
              ...
              <div className="relative overflow-hidden rounded-xl border border-border shadow-card">
                <div className="bg-gradient-to-r from-primary to-primary/70 p-6 text-primary-foreground">
                  ...
                  <div className="text-sm opacity-90">Discover nearby spots and community favorites</div>
              ...
              <div className="space-y-3">
                {["Local bakery offers 10% off", "Community yoga this Sunday", "New group: Pet Lovers"]
                  .map((title, idx) => (
                    <div key={idx} className="bg-white/70 dark:bg-black/30 backdrop-blur-md border border-border rounded-xl shadow-card p-4">
